...
class CalculateParasAndFLOPs(object):
    ...
    def conv_hook3(self, *inputs): #conv_hook3:用户可以自行决定拦截了中间信息之后要做什么
    ...
    def Calculate(self, net):
        ...
        if not childrens:
            if isinstance(net, torch.nn.Conv3d): #判断一个对象是否是一个已知的类型
                net.register_forward_hook(self.conv_hook3)  #对需要的层注册hook,hook（）函数是register_forward_hook()函数必须提供的参数
