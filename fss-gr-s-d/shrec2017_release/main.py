#M-FSS-GR:main
#main_3_2a3.py的基础上：新增测性能（用profiler）
...
class Processor():
    def __init__(self, arg):
        ...
        self.recoder2 = Recorder(work_dir=self.path_performance,
                                 print_log=self.arg.print_log)  # 这个recoder2用来记录性能评估器profiler的输出,work_dir是log_dir下results子文件夹，默认输出txt就是results里的log.txt
        # 上新增：定义recoder2
    ...
    def start(self):
        ...
        if self.arg.phase == 'train' and MODALITY == "dynamic":
            ...
            with tf.Graph().as_default():
                ...
                params = profiler.profile_name_scope(
                    options=profile_scope_opt_builder.build())  # 这条语句显示Options和Model Analysis Report
                flops = tf.profiler.profile(graph=sess.graph, run_meta=run_metadata, cmd='op',
                ...
                self.recoder2.print_log('GFLOPs: {} G'.format(flops.total_float_ops / (10 ** 9)),
                                        print_time=False)
                self.recoder2.print_log('Trainable Params: {}'.format(params.total_parameters),
                                        print_time=False)
                self.recoder2.print_log('Trainable Params: {}M'.format(params.total_parameters / (10 ** 6)),
                                        print_time=False)
    ...
    # 上改： 模型性能分析profile 4
        ...
        for batch_idx, data in enumerate(loader):
            ...
            if (batch_idx + 1) % 35 == 0:  # 每35步(每35个batch)统计一次数据(1960/batch_size=1960/8=245 1960/4=490)
                ...
                self.recoder2.print_log(str="=========================timeline=============================",
                                        print_time=False)
                ...
                summary, step, _, loss_val, pred_val, acc_val = sess.run([ops['merged'], ops['step'],ops['train_op'], ops['loss'], ops['pred'],ops['accuracy']], feed_dict=feed_dict)
            #性能分析5
