...
"""
#M-FSS-GR'&M-FSS-GR
#go to line 205 and change (mlp,mlp2,mlp3) to change model
#思路3_3：在思路3_2基础上修改，最原先shrec2017_release/model_cls_dynamic.py（现改名为model_cls_dynamic_orig）
#思路3_3:3分支，动态分支-场景流：修改SA模块，然后输入只有3维场景流
...
def get_model(point_cloud, datasource,reconflow,num_frames, is_training, bn_decay=None, CLS_COUNT=-1):
    ...
    reconflow_re_new = tf_util.conv1d(reconflow_re,flow2_channel, 1, padding='VALID', stride=1,
                               ...
                               scope='reconflow_toflow2_channel_dynamic', bn_decay=bn_decay,
                               data_format='NHWC')#reconflow_re最后一个通道变得和flow2一样（NHWC）
    reconflow_re_new_4d = tf.reshape(reconflow_re_new, [batch_size, 32,-1,flow2_channel])#4维
    flow2_new0 = tf.reshape(flow2, [batch_size, 32, -1, flow2_channel])  # 4维
    numpoint_frame=flow2_new0.get_shape()[2].value#16
    cat_rec_flow2=tf.concat([flow2_new0, reconflow_re_new_4d], 2)#[bs,32,16+16,flow2_channel]
    ...
    flow2_new=tf_util.conv2d(cat_rec_flow2_trans, numpoint_frame,[1, 1],
                   ...
                   data_format='NCHW')#变为NCHW形式#[bs,16,32,flow2_channel]  #思路2.2-2.4：改1：BN改为True
    ...
    flow2_new2=tf_util.conv1d(tf.concat([flow2_new, l2_time], -1),
                   ...
                   data_format='NHWC')    ##思路2.2-2.4：BN改为True

    #flow = flow2 #原flow
    #flow = flow2_new2       #思路2.1
    flow = flow2_new2+reconflow_re_new #思路2.4：改2
    #上改8
    ...
    #l2_xyz = l2_xyz_new2       #思路2.1
    #l2_xyz = l2_xyz_new2+reconflow_re #思路2.4：改3
    l2_xyz = l2_xyz_new2+l2_xyz_new  #思路2.3：改3
    #l2_xyz = l2_xyz_new2+l2_xyz_new+reconflow_re #实验10：思路2.4：改3+思路2.3：改3
    ...
    #思路3：和静态分支static_net, 原动态分支flow_net并列，第三个分支：由场景流信息构成分支
    #思路3_3:参考SA，写第三分支。基于SA写changed_pointnet_sa_module：多层tf_util.conv2d，maxpool，再接tf_util.conv2d
    # reconflow_re = tf_util.batch_norm_for_conv1d(reconflow_re, is_training=is_training,
    #                                 bn_decay=bn_decay, scope='bn3_3_dynamic',
    #                                 data_format='NHWC', freeze_bn=False)  #归一化
    scene_flow = changed_pointnet_sa_module(reconflow_re,  npoint=None,
                                            mlp=[32, 64],mlp2=[128,256],mlp3=[512,1024], is_training=is_training,
                                            bn=True, bn_decay=bn_decay, scope='scene3_3_layer_dynamic')
    # M-FSS-GR':实验3_3_4的mlp:mlp=[32,128],mlp2=[512,1024],mlp3=None
    # 实验3_3_7的mlp:mlp=[16,32],mlp2=[64,128]，mlp3=[256,512]
    # M-FSS-GR:实验3_3_9的mlp:mlp=[32, 64],mlp2=[128,256],mlp3=[512,1024]
    #reconflow_re:[batch_size,32*16,3]
    #changed_pointnet_sa_module:两层tf_tuil.conv2d,通道数mlp.再经过maxpool.最后再经过两层tf_tuil.conv2d，mlp2=[512,1024]
    #scene_flow：(batch_size,1,1024)
    #上思路3_3:参考SA，写第三分支。基于SA写changed_pointnet_sa_module：多层tf_util.conv2d，maxpool，再接tf_util.conv2d

    # Fully connected layers
    scene_flow_net = tf.reshape(scene_flow, [batch_size, -1])  # shape(batch_size,1024)
    scene_flow_net = tf_util.fully_connected(scene_flow_net, CLS_COUNT, activation_fn=None, scope='scene_fc3_dynamic')
    # 本质scene_flow_net=scene_flow_net*w+b 。 (二维矩阵flow_net)
    # scene_flow_net:shape(batch_size,CLS_COUNT)
    return static_net, flow_net, scene_flow_net,end_points   #思路3
    #return static_net, flow_net, scene_flow_net,end_points  #思路2
