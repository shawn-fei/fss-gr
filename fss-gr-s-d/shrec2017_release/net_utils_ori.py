...
def pointnet_sa_module(xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope,
                       bn=True, pooling='max', knn=False, use_xyz=True, use_nchw=False, freeze_bn=False,
                       sample_idx=None, trainable=True):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        if group_all:
            ...
            new_xyz, new_points, idx, sample_idx, grouped_xyz = sample_and_group_by_index(npoint, radius, nsample, xyz,
                                                                                          points, knn, use_xyz,
                                                                                          sample_idx)
        else:
            new_xyz, new_points, idx, sample_idx, grouped_xyz = sample_and_group(npoint, radius, nsample, xyz, points,
                                                                                 knn, use_xyz)
...
def static_module(xyz, time, points, npoint, radius, nsample, mlp, mlp2, group_all,
                  is_training, bn_decay, scope, module_type='ind_without_time', fps=True, bn=True, pooling='max',
            ...
            if use_xyz:
                if module_type == 'ind':
                    new_points = tf.concat([grouped_xyz, grouped_time, grouped_points],
                                           axis=-1)  # (batch_size, npoint, nample, 3+1+channel)
                    ...
                    new_points = tf.concat([grouped_xyz, grouped_time, grouped_points, new_points_expand],
                                           axis=-1)  # (batch_size, npoint, nample, 3+1+channel+channel)
            else:
                new_points = grouped_points
        else:
            new_points = grouped_xyz
...
def get_st_surface(GROUP_SIZE, batch_size, duplicate_grouped_time, flow_dim, new_points_, npoint, nsample, W=None):
    EPS = 0.000001
    new_points_ = tf.concat(
        [new_points_, tf.ones(shape=[batch_size, npoint, flow_dim // GROUP_SIZE, nsample, 1], dtype=new_points_.dtype)],
        -1)
...
def dynamic_module(xyz, time, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope,
    ...
    with tf.variable_scope(scope) as sc:
        ...
        new_time = gather_point(time,
                                sample_idx)  # (batch_size, npoint, 1)  比如：一个batch,npoint（32*32）就是一列：值是32*32个点对应的帧是第几帧.
        time_ = tf.reshape(time, [batch_size, 1,
                                  -1])  # (batch_size, 1, ndataset) #比如：一个batch,ndataset（32帧*128点）就是一行：128个0（帧time索引）,128个1,128个2，……，128个31.
        ...
        idx, pts_cnt = query_ball_point_var_rad(radius_, nsample, xyz,
                                                new_xyz)  # 以new_xyz中npoint个点为查找点，在原始xyz的ndataset中找nsample个点。查找范围是球形搜索区域（查找点为球心，radius为半径）
        ...
        for i, num_out_channel in enumerate(mlp):  # layer1：mlp=[64, 128]。layer2:mlp=[128, 256],
            new_points = tf_util.conv2d(new_points, num_out_channel, [1, 1],
                                        padding='VALID', stride=[1, 1],
                                        bn=bn, is_training=False, trainable=False, freeze_bn=True,
            ...
            # layer1，i=0：new_points的shape:,(batch_size,npoints,nsample,64)即（batch_size,32*32,64,64）
            ...
            new_points_ = tf.concat([new_points_, grouped_xyz], -1)  # shape:(batch_size,npoints,nsample,flow_dim)
            ...
            duplicate_grouped_time = tf.tile(grouped_time, [1, 1, 1,
                                                            flow_dim // GROUP_SIZE])  # shape:(batch_size, npoint, nsample,flow_dim // GROUP_SIZE) flow_dim // GROUP_SIZE是组数（就是把flow分为几组）
            # duplicate_grouped_time:grouped_time(batch_size, npoint, nsample,1)最后一个维度（邻居点对应时间）复制flow_dim // GROUP_SIZE次。
            duplicate_grouped_time = tf.reshape(duplicate_grouped_time,
                                                [batch_size, npoint, nsample, flow_dim // GROUP_SIZE, 1])
            # shape:(batch_size, npoint, nsample,flow_dim // GROUP_SIZE,1) 第4个维度：比如每个查找点的邻居对应的时间复制11次（每组特征11维度）
            new_points_ = tf.transpose(new_points_,
                                       [0, 1, 3, 2, 4])  ##转置：shape:(batch_size,npoints,特征组数，nsample，每组包含特征数GROUP_SIZE)
            duplicate_grouped_time = tf.transpose(duplicate_grouped_time, [0, 1, 3, 2,
                                                                           4])  # (batch_size, npoint, flow_dim // GROUP_SIZE,nsample,1)
            # new_points_:每个search point(共npoints)的nasmple个邻居的33(flow_dim)个特征，然后将这33个特征分为11组，每组3(GROUP_SIZE)个特征。最后就是（nsample个邻居的3个特征   nsample个邻居的另3个特征 …… ）
            ...
            if i == 0 and last_W is None:
                n_new_points_ = get_st_surface(GROUP_SIZE, batch_size, duplicate_grouped_time, flow_dim, new_points_,
                                               npoint, nsample)
            else:
                if i == 0:
            ...
            normals_feat = tf.reshape(n_new_points_, [batch_size, npoint, flow_dim // GROUP_SIZE,
                                                      GROUP_SIZE + 1])  # 比如(batch_size,32*32,11,4)
            normals_feat = normals_feat[:, :, :, 0:GROUP_SIZE]  # 比如(batch_size,32*32,11,3)
            normals_feat = tf.concat([normals_feat, -tf.ones(shape=[batch_size, npoint, flow_dim // GROUP_SIZE, 1],
                                                             dtype=normals_feat.dtype)],
                                     -1)  ##比如(batch_size,32*32,11,3+1),3+1中1那列都是-1
            ...
            new_points_normals = tf.reshape(n_new_points_, [batch_size, npoint, flow_dim // GROUP_SIZE, 1,
                                                            GROUP_SIZE + 1]) * tf.concat([new_points_, tf.ones(
                shape=[batch_size, npoint, flow_dim // GROUP_SIZE, nsample, 1], dtype=new_points_.dtype)], -1)
            ...
            n_new_points_ = tf.concat([n_new_points_, -tf.ones(shape=[batch_size, npoint, flow_dim // GROUP_SIZE, 1],
                                                               dtype=n_new_points_.dtype)], -1)
...
def flow_merge_across_res(flow1_list, flow2_list, flow1_idx, flow2_idx, flow1_time, flow2_time, l1_xyz, l2_xyz, scope,
                          is_training, bn_decay, bn=False, data_format='NHWC'):
