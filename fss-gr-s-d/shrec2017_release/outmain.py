        ...
        if self.arg.phase == 'train' and MODALITY == "static":
            ...
            with tf.Graph().as_default():
                ...
                with tf.device('/gpu:' + str(GPU_INDEX)):
                    pointclouds_pl, labels_pl = MODEL.placeholder_inputs(BATCH_SIZE, NUM_POINT, NUM_FRAME, INPUT_DIM)#模型（点云和标签）占位（placeholder），分配必要的内存
                    ...
                    batch = tf.compat.v1.get_variable('batch', [],
                                            initializer=tf.compat.v1.constant_initializer(0), trainable=False)#用于获取已存在的变量,不存在就新建一个
                ...
                if pretrained is not '':
                    ...
                    loading_saver = tf.compat.v1.train.Saver(variables_to_restore)
        ...
        if self.arg.phase == 'train' and MODALITY == "dynamic":
            ...
            with tf.Graph().as_default():
                ...
                ops = {'pointclouds_pl': pointclouds_pl,
                       'labels_pl': labels_pl,
                       'is_training_pl': is_training_pl,
                       'pred': tf.tuple(tensors=[static_pred, flow_pred]),
                       'loss': total_loss,
                       'accuracy': accuracy,
                       'train_op': train_op,
                       'merged': merged,
            ...
            with tf.Graph().as_default():
                ...
                # Init variables
                loading_saver = tf.compat.v1.train.Saver()
                loading_saver.restore(sess, MODEL_PATH)
                print("The model has been loaded !!!!!!!!!!!!!")
                # Add summary writers
                merged = tf.compat.v1.summary.merge_all()
                writer = tf.compat.v1.summary.FileWriter(os.path.join(LOG_DIR, 'sess'), sess.graph)
