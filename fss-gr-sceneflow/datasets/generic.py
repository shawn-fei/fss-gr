...
class Batch:
    def __init__(self, batch):
        ...
        for key in ["sequence", "ground_truth", "orig_size","idx_89600_sample"]:
            #print("key",key)
            self.data[key] = []
            if key in ["sequence", "ground_truth", "orig_size"]:
                ...
                for ind_batch in range(batch_size):
                    #"89599_2799_14_2_28_10"
                    l=batch[ind_batch][key].tolist()
                    l = l.split("_")
                    l = list(map(int, l))
                    l_tensor = torch.IntTensor(l)
                    self.data[key].append(l_tensor)
...
class SceneFlowDataset(Dataset):
    ...
    def __getitem__(self, idx):
        sequence, ground_truth, orig_size,idx_89600_sample = self.to_torch(*self.subsample_points_rnd(*self.load_sequence(idx)))
        data = {"sequence": sequence, "ground_truth": ground_truth, "orig_size": orig_size,"idx_89600_sample":idx_89600_sample}

        return data

    def to_torch(self, sequence, ground_truth, orig_size,idx_89600_sample):
        """
        ...
        return sequence, ground_truth, orig_size,idx_89600_sample

    def subsample_points_rnd(self, sequence, ground_truth,idx_89600_sample):
        """
    ...
    def load_sequence(self, idx):#datasets/shrec_flownet3d.Shrec类内重写此方法
        """
