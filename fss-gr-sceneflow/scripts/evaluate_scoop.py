...
def eval_model(scoop, testloader, log_file, log_dir, res_dir, args):
    ...
    #############################
    # SCOOP_2:下改3:定义Recorder类对象record3和record4，用此类方法记录est_flow及其数据来源。
    # 1.记录est_flow文件路径
    path_log_est_flow = "../est_flow/"  # est_flow日志文件写到：SCOOP/est_flow/log.txt
    try:  # 下面是可能会产生异常的情况
        os.makedirs(path_log_est_flow)  # 创建多层目录
    except FileExistsError:  # 若创建多层目录时已存在，则触发此异常
        pass  # 异常处理，pass：忽略错误继续运行
    print("开始将est_flow写到txt")
    print("est_flow保存至{}log.txt".format(path_log_est_flow))
    # 调用Recorder类方法print_log，将flow输入写入txt
    record3 = Recorder(work_dir=path_log_est_flow, print_log=True)
    print("!!!请问之前生成的{}log.txt删除了吗".format(path_log_est_flow))

    #2. 调用Recorder类方法print_log，将每条flow对应的数据来源写到txt
    path_log_datasource = "../data_source_est_flow/"  # est_flow对应数据来源写到：SCOOP/data_source_est_flow/log.txt
    print("开始将est_flow对应数据来源写到txt")
    print("est_flow对应数据来源保存至{}log.txt".format(path_log_datasource))
    try:  # 下面是可能会产生异常的情况
        os.makedirs(path_log_datasource)  # 创建多层目录
    except FileExistsError:  # 若创建多层目录时已存在，则触发此异常
        pass  # 异常处理，pass：忽略错误继续运行
    record4 = Recorder(work_dir=path_log_datasource, print_log=True)

    # 下record6:记录test,performance ，日志文件写到：SCOOP/data_performance_test/log.txt
    path_log_performance_test = "../data_performance_test/"  ## 日志文件写到：SCOOP/data_performance_test/log.txt
    try:  # 下面是可能会产生异常的情况
        os.makedirs(path_log_performance_test)  # 创建多层目录
    except FileExistsError:  # 若创建多层目录时已存在，则触发此异常
        pass  # 异常处理，pass：忽略错误继续运行
    record6 = Recorder(work_dir=path_log_performance_test, print_log=True)
    # 以上： record6:记录test,performance
    #SCOOP_2:上改3：定义Recorder类对象record3和record4，用此类方法记录est_flow及其数据来源。
    #############################
    for it, batch in enumerate(tqdm(testloader)):
        ...
        if args.all_candidates:
            ...
            with torch.no_grad():
                est_flow, corr_conf, _, graph = scoop(batch["sequence"])#est_flow和训练阶段recon_flow的shape一样，和batch["sequence"]中PC1的shape一样，（batch_size,nb_points,3）,比如nb_points=16
                # 以下：是为测FLOPs,Params
                flops, params = thop.profile(scoop, inputs=(batch["sequence"],))
                record6.print_log("test,batch_id={},GFLOPs:{} G".format(it, flops / (10 ** 9)))  # 打印计算量（以十亿次浮点运算为单位）
                record6.print_log("test,batch_id={},Params:{} M".format(it, params / (10 ** 6)))  # 打印参数量（以百万为单位）
                # 以上：是为测FLOPs,Params
        ...
        if args.use_test_time_refinement:
            ...
            est_flow = refined_flow.detach()#estimated flow


            if it==0:print("est_flow的shape:",est_flow.shape)

            ##################
            #scoop_3下修改4：est_flow及其数据来源写到txt
            # 将est_flow（batch_size,16,3）转换为（batch_size,48），然后按batch_size逐行写入txt
            new_est_flow = est_flow.reshape(est_flow.shape[0], (est_flow.shape[1] * est_flow.shape[2]))  # （batch_size,22,3）转换为（batch_size,66）
            for i in range(new_est_flow.shape[0]):  # 逐行写入txt。i:batch内第几个
                list_est_flow = new_est_flow[i].tolist()  # tolist:array(48)转为list
                list_est_flow = map(str, list_est_flow)  # map将list_est_flow中每个元素转为str
                str_est_flow = ' '.join(list_est_flow)  # join:list_est_flow每个元素中间用空格隔开，转为str
                record3.print_log(str_est_flow, print_time=False)  # 写入日志，但不输出时间
                # 将est_flow对应数据来源写入recorder4对应日志
                idx = batch["idx_89600_sample"][i]  # est_flow对应数据来源
                idx_list = idx.tolist()
                idx_list = map(str, idx_list)
                str_idx_list = ' '.join(idx_list)
                record4.print_log(str_idx_list, print_time=False)  # 写入recorder4对应日志，但不输出时间

                idx_sample = it * new_est_flow.shape[0] + i + 1  # 第几个样本
                print("第{}/89600行写入txt".format(idx_sample))  # 不写入日志，只输出

            # scoop_3上修改4：est_flow及其数据来源写到txt
            ##################
...
def my_main(args):
    ...
    if args.dataset_name.split("_")[0].lower() == "HPLFlowNet".lower():
        ...
        ##################################
        #SHREC:(SCOOP_2：改1)以下代码新增：如果数据集是shrec
        # path2data，也是"Project_root/data/FlowNet3D/Process_NPZ_scoop_SHREC"
        if args.dataset_name.split("_")[1].lower() == "shrec".lower():
            path2data = os.path.join(path2data, "Process_NPZ_scoop_SHREC")# SHREC_npz数据集所在路径
            from datasets.shrec_flownet3d import Shrec   #？新建1

            assert args.mode == "val" or args.mode == "test" or args.mode == "all", "Problem with mode " + args.mode
            #dataset
            dataset = Shrec(root_dir=path2data, nb_points=args.nb_points, all_points=args.all_points,
                            same_v_t_split=True, mode=args.mode) #mode="test",该值来自scripts/evaluate_on_shrec.sh中参数mode。如果mode改为all,就是全体数据集（89600）

        # SHREC:(SCOOP_2：改1)以上代码新增：如果数据集是shrec
        ######################################
