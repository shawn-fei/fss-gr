...
def compute_loss_unsupervised(recon_flow, corr_conf, target_pc_recon, graph, batch, args):
    ...
    if args.use_corr_conf and args.corr_conf_loss_weight > 0:#--use_corr_conf 1 --corr_conf_loss_weight 0.1#在train_on_kitti_v.sh
        if mask is not None:
    ...
    if args.use_smooth_flow and args.smooth_flow_loss_weight > 0: #use_smooth_flow=1,smooth_flow_loss_weight=10#见train_scoop.py
